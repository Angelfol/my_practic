...
Написан на **C++17** в архитектуре **CGI + MySQL + серверный рендеринг HTML** — под shared-хостинг Beget (`http://iiolkhtc.beget.tech/UTP2026/`), где нельзя держать постоянно работающий процесс: Apache запускает CGI-программу на каждый запрос. Интерфейс оформлен в стиле терминала (TUI) со шрифтом Iosevka Term.
