...
Написан на **C++17** в архитектуре **CGI + MySQL + серверный рендеринг HTML** — под shared-хостинг Beget (`http://iiolkhtc.beget.tech/UTP2026/`)
...
Без фреймворков, ORM и внешних зависимостей
...
`libmariadb.dll` кладётся рядом с `*.cgi`. Пустой `CRYPTO_LIBS` включает встроенную реализацию SHA-256 (дайджесты совпадают с libcrypto).
